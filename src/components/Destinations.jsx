  ...
  return (
    ...
    >
      <div
        ...
        style={{
          transform: `perspective(1000px) rotateX(${rotation.x}deg) rotateY(${rotation.y}deg) ${isSelected ? 'scale(0.98)' : isHovered ? 'scale(1.02)' : 'scale(1)'}`,
          ...
          boxShadow: isHovered
            ? `0 20px 60px ${dest.glowColor}, 0 0 40px ${dest.glowColor}`
            : isSelected
      ...
      >
        ...
        <div className="relative h-56 overflow-hidden">
          {/* Gradient background always present (shows through or as fallback) */}
          <div
            className="absolute inset-0"
            style={{
              background: dest.id === 'egypt'
                ? 'linear-gradient(135deg, #2a1e00 0%, #1a1200 30%, #0a0d1a 60%, #1a3a5c 100%)'
                : dest.id === 'rome'
                ? 'linear-gradient(135deg, #2a0f00 0%, #1a0500 30%, #0a0d1a 60%, #4a3728 100%)'
                : 'linear-gradient(135deg, #140a1a 0%, #0a0514 30%, #0a0d1a 60%, #1a2a3a 100%)',
            }}
          />

          {/* Real image if available, emoji fallback otherwise */}
          {imageSrc && !imgError ? (
            <img
              src={imageSrc}
              alt={dest.name}
            ...
            <>
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-8xl opacity-20" style={{ filter: 'blur(4px)' }}>
                  {dest.emoji}
                </span>
              </div>
              <div className="absolute inset-0 flex items-center justify-center">
          ...
          <div className="absolute top-4 left-4 flex gap-2 flex-wrap">
            <span
              className={`font-inter text-xs px-3 py-1 rounded-full border ${riskColors[dest.risk]}`}
            >
              Risque {dest.risk}
            </span>
            <span className="font-inter text-xs px-3 py-1 rounded-full bg-white/5 border border-white/10 text-gray-300">
              {dest.duration}
            </span>
          </div>
